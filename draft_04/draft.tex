...
\enddata
\tablecomments{Effective temperatures and photospheric diameters are from \cite{1993AA...270..315D}. Distances are from \cite{2007A&A...474..653V}. Mass loss rates are from \cite{1985pssl.proc..351D} and \cite{1998ApJ...503..396R}.}
...
\tabletypesize{\scriptsize}
\tablecaption{Radio Spectral Indices and Resulting Density and Temperature Coefficients.}
\tablehead{	\colhead{}		       				& 
			\colhead{Spectral}				&
			\colhead{Density}				&
			\colhead{Temperature}				\\
...
\tablecaption{Ionized mass loss rates for $\alpha$ Boo and $\alpha$ Tau.}
\tablehead{	\colhead{Star}		       				& 
			\colhead{Wavelength (cm)}				&
			\colhead{$\rm{\dot{M}_{ion}}$ (M$_{\odot}$ yr$^{-1}$)}}
